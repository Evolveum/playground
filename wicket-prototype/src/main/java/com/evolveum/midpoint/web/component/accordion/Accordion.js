...
function getAccordion(accordionId){return document.getElementById(accordionId);}
function getPanel(accordionControl,child){return child.getElementsByTagName(accordionControl);}

TINY.accordion=function(){
	function slider(accordionName){this.accordionName=accordionName; this.accordions=[];}
	...
	slider.prototype.expand=function(state,component){
		for(var i=0;i<this.length;i++){
			...
			if((currentHeight != 1 && content.opened == -1) && (state == 1 || i == component)){
				...
				content.opened=1; 
				header.className=this.selectedAttr; timer(content,1);
	...
	function timer(content){content.timer=setInterval(function(){slide(content);},20);}
	function slide(content){
		...
		if((content.opened == 1 && totalHeight >= content.totalHeight) || (content.opened != 1 && totalHeight==1 )){if(content.opened == 1){content.style.height='auto';} clearInterval(content.timer);}
	};
